...
Mersal-rabbitmq uses `uv <https://docs.astral.sh/uv/>`_ for dependency
...
mersal-rabbitmq follows `Semantic Versioning <https://semver.org/>`_.
...
3. Go to `Actions -> Build & Publish Package
   <https://github.com/mersal-org/mersal-rabbitmq/actions/workflows/publish.yml>`_,
...
4. `.github/workflows/publish.yml <.github/workflows/publish.yml>`_ then:
   ...
   - and only once that succeeds, tags the commit ``v1.2.4`` and creates the
     `GitHub Release <https://github.com/mersal-org/mersal-rabbitmq/releases>`_ with
     ``--generate-notes``.
...
`Test PyPI <https://test.pypi.org/project/mersal-rabbitmq/>`_ under its dev
version (e.g. ``1.2.4.dev3``):

.. code-block:: shell

   pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple mersal-rabbitmq
